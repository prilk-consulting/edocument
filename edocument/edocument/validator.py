...
def get_xml_validator(xml_bytes, edocument_profile):
	...
	if edocument_profile.validator_path:
		try:
			...
			frappe.log_error(f"Error loading validator from path {edocument_profile.validator_path}: {str(e)}")
...
def validate_xml_structure(xml_bytes: bytes) -> bytes:
	...
	try:
		...
		error_msg = f"XML structure validation failed: {str(e)}"
...
def validate_xml_against_xsd_file(xml_bytes: bytes, xsd_file_path: Path | str) -> bytes:
	...
	try:
		...
		error_msg = f"XSD validation failed: {str(e)}"
		frappe.log_error(error_msg, "XSD Validation")
		raise ValueError(error_msg)
	except Exception as e:
		error_msg = f"XSD schema load or validation failed: {str(e)}"
		frappe.log_error(error_msg, "XSD Validation")
		raise ValueError(error_msg)


def validate_xml_against_schematron_file(xml_bytes: bytes, xsl_file_path: Path | str) -> tuple[list[str], list[str]]:
	"""
...
def validate_xml_against_schematron_files(xml_bytes: bytes, xsl_file_paths: list[Path | str]) -> tuple[list[str], list[str]]:
	"""
