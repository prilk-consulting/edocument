...
class PEPPOLGenerator:
    ...
    def __init__(self, invoice):
        ...
        self.document_elements = DOCUMENT_TYPE_ELEMENTS.get(self.document_type, DOCUMENT_TYPE_ELEMENTS['Invoice'])
    ...
    def _set_header(self):
        ...
        invoice_type_code = self.get_invoice_type_code(self.invoice)
        type_code_elem = ET.SubElement(self.root, f"{{{self.namespaces['cbc']}}}{self.document_elements['type_code']}")
        ...
        if self.document_type == 'CreditNote' and self.invoice.return_against:
            ...
            invoice_doc_ref = ET.SubElement(billing_ref, f"{{{self.namespaces['cac']}}}InvoiceDocumentReference")
            ...
            try:
                original_invoice_date = frappe.db.get_value("Sales Invoice", self.invoice.return_against, "posting_date")
                if original_invoice_date:
                    invoice_issue_date = ET.SubElement(invoice_doc_ref, f"{{{self.namespaces['cbc']}}}IssueDate")
    ...
    def _set_seller(self):
        ...
        if self.seller_address:
            ...
            if self.seller_address.address_line2:
                additional_street = ET.SubElement(postal_address, f"{{{self.namespaces['cbc']}}}AdditionalStreetName")
        ...
        if self.seller_contact:
            ...
            if self.seller_contact.first_name or self.seller_contact.last_name:
                ...
                contact_name.text = f"{self.seller_contact.first_name or ''} {self.seller_contact.last_name or ''}".strip()
    ...
    def _set_buyer(self):
        ...
        if self.buyer_contact:
            ...
            if self.buyer_contact.first_name or self.buyer_contact.last_name:
                contact_name = ET.SubElement(contact, f"{{{self.namespaces['cbc']}}}Name")
                contact_name.text = f"{self.buyer_contact.first_name or ''} {self.buyer_contact.last_name or ''}".strip()
    ...
    def _add_line_item(self, root: ET.Element, item):
        ...
        quantity_elem_name = self.document_elements['quantity']  # InvoicedQuantity, CreditedQuantity, DebitedQuantity
    ...
    def _get_bank_details(self, mode_of_payment: str, company: str) -> tuple[str | None, str | None]:
        ...
        bank_account_name = frappe.db.get_value(
            "Bank Account", 
            {"account": account, "company": company, "is_company_account": 1, "disabled": 0}
        )
    ...
    def _add_allowances_charges(self):
        ...
        if discount_amount > 0:
            ...
            if self.invoice.total > 0:
                ...
                multiplier_factor = ET.SubElement(allowance_charge, f"{{{self.namespaces['cbc']}}}MultiplierFactorNumeric")
    ...
    def get_seller_electronic_address(self, company, seller_contact=None) -> dict[str, str] | None:
        ...
        if company.edocument_electronic_address_scheme and company.edocument_electronic_address:
            scheme_id = frappe.db.get_value("Common Code", company.edocument_electronic_address_scheme, "common_code")
            if scheme_id:
    ...
    def get_buyer_electronic_address(self, customer, invoice, buyer_contact=None, buyer_address=None) -> dict[str, str] | None:
        ...
        if customer.edocument_electronic_address_scheme and customer.edocument_electronic_address:
            scheme_id = frappe.db.get_value("Common Code", customer.edocument_electronic_address_scheme, "common_code")
            if scheme_id:
