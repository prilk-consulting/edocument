...
def parse_peppol_xml(xml_bytes, edocument_profile):
	...
	try:
		...
		if document_type == 'CreditNote':
			billing_ref_id = get_xml_text(root, './/cac:BillingReference/cac:InvoiceDocumentReference/cbc:ID', namespaces)
			if billing_ref_id:
		...
		pi_data["items"] = parse_peppol_line_items(root, namespaces, pi_data.get("purchase_order"), pi_data.get("supplier"), document_elements)
		...
		frappe.log_error(f"PEPPOL Parsing Error: {str(e)}", "EDocument PEPPOL Parsing")
		raise
...
def parse_peppol_seller(root, namespaces):
	...
	# Seller name
	seller_name = get_xml_text(seller_party, './/cac:PartyLegalEntity/cbc:RegistrationName', namespaces) or \
				 get_xml_text(seller_party, './/cac:PartyName/cbc:Name', namespaces)
...
def parse_peppol_buyer(root, namespaces):
	...
	# Buyer name
	buyer_name = get_xml_text(buyer_party, './/cac:PartyLegalEntity/cbc:RegistrationName', namespaces) or \
				 get_xml_text(buyer_party, './/cac:PartyName/cbc:Name', namespaces)
	...
	for invoice_line in root.findall(f'.//cac:{line_elem_name}', namespaces):
		...
		if product_name:
			if len(product_name) > 140:
				...
				item["description"] = product_name + (" | " + product_description if product_description else "")
			else:
		...
		buyer_item_id = get_xml_text(invoice_line, './/cac:Item/cac:BuyersItemIdentification/cbc:ID', namespaces)
...
def parse_peppol_payment_terms(root, namespaces, default_due_date=None):
	...
	for payment_term in root.findall('.//cac:PaymentTerms', namespaces):
		...
		discount_basis_date = get_xml_text(payment_term, './/cac:PaymentDiscountTerms/cbc:BasisDate', namespaces)
	...
	for pi_item in pi_data.get("items", []):
		...
		if pi_item.get("po_detail"):
			try:
				if frappe.db.exists(
					"Purchase Order Item",
					{"name": pi_item["po_detail"], "parent": pi_data["purchase_order"]}
				):
					continue
			except Exception:
				pass
